  ...
  <div id="fb">
    <div id="fb-top">
      <p>
        ...
        <span class="top-links">Find Friends • Settings</span>
      </p>
    </div>
    ...
    <div v-else class="requests-container">
      ...
      >
        ...
        <div class="friend-info">
          <p class="friend-name">{{ request.sender?.name || 'Unknown User' }}</p>
          <p class="friend-email">{{ request.sender?.email || 'No Email' }}</p>
        </div>

        <div class="friend-actions">
          <button class="accept-btn" @click="acceptRequest(request.id)">Accept</button>
          <button class="decline-btn" @click="declineRequest(request.id)">Decline</button>
        </div>
      </div>
    </div>
  </div>
    ...
    return;
      ...
      try {
        const response = await axios.post(
              `http://localhost:5000/api/friend/accept/${requestId}`,
              {}, // Empty body
              {
                headers: {
